...
/* ============================================
 * SCCP → TCAP extraction (STRICT)
 * ============================================ */
static uint8_t *extract_tcap(uint8_t *d, int len, int *out_len) {
    if (!d || len < 6)
        return NULL;
    ...
    uint8_t tcap_len = d[ptr];

    if (ptr + 1 + tcap_len > len)
        return NULL;

    uint8_t *tcap = &d[ptr + 1];

    /* validate TCAP tag */
    if (tcap[0] != 0x62 && tcap[0] != 0x64 &&
        tcap[0] != 0x65 && tcap[0] != 0x67) {
        return NULL;
    }
 ...
 * MAIN ROUTER
 * ============================================ */
void route_tcap(msg_t *msg, uint32_t _otid, uint32_t _dtid, int _type) {

    int direction = msg->cb[0];

    /* ============================================
     * ALWAYS re-extract and re-parse TCAP
     * ============================================ */
    int tcap_len = 0;
    uint8_t *tcap = extract_tcap(msg->data, msg->len, &tcap_len);

    if (!tcap) {
        printf("DROP: invalid SCCP/TCAP\n");
        msg_pool_put(msg);
        return;
    }
    ...
    printf("TCAP parsed → type=%d otid=%u dtid=%u\n", type, otid, dtid);
    ...
    if (direction == DIR_FROM_BACKEND) {
        ...
        if (type == 1 && otid != 0) {
            ...
            if (extract_calling_gt(msg->data, msg->len, gt, &gt_len) < 0)
                gt_len = 0;

            tx_store_full(otid, backend_fd, gt, gt_len);
